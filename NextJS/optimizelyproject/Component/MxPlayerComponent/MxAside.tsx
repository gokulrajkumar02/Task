...
const MxAside = ({ openSlideBar }: Props) => {

  const router = useRouter()
  return (
    <div className="hidden md:flex mt-6">
      <div className="w-[250px] flex flex-col h-[60%] ml-8">
        <div className="flex flex-col h-[80%] w-[80%] justify-between"
        >
            {slideBarContent.map((item,index) => {
              return (
               
                <div key={index} className="flex  gap-x-10 py-2 px-1 items-center hover:bg-gray-200 hover:rounded-md hover:cursor-pointer"  onClick={() => router.push(item.NavigayeTo)}>
                  <label className="h-8 w-8 hover:cursor-pointer">{item.svg}</label>
                  <label className="text-[20px] font-semibold hover:cursor-pointer select-none">{item.Title}</label>
                </div>
              );
            })}
          </div>
